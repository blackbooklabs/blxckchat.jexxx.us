...
import { generateDeviceCode, generateCodeVerifier, generateCodeChallenge, isTokenValid, shouldRefreshToken, getTokenExpiryMinutes, startDeviceAuth, pollDeviceAuth, refreshAccessTokenViaServer, } from "../lib/auth.js";
...
async function withMockSecureServer(handler, fn) {
...
test("startDeviceAuth posts codeChallenge and returns a verificationUrl", async () => {
    await withMockSecureServer((req, res) => {
        const body = req
            .parsedBody;
...
test("pollDeviceAuth resolves credentials once status is consumed", async () => {
    let callCount = 0;
    await withMockSecureServer((_req, res) => {
        callCount++;
        if (callCount < 2) {
...
test("pollDeviceAuth throws when the user denies authorization", async () => {
    await withMockSecureServer((_req, res) => sendJson(res, 200, { status: "denied" }), async () => {
        await assert.rejects(() => pollDeviceAuth("ABCD1234", "verifier", 30, 10), /denied/i);
    });
});
test("pollDeviceAuth throws when the device code expires", async () => {
    await withMockSecureServer((_req, res) => sendJson(res, 200, { status: "expired" }), async () => {
        await assert.rejects(() => pollDeviceAuth("ABCD1234", "verifier", 30, 10), /expired/i);
    });
});
...
test("refreshAccessTokenViaServer throws on server error", async () => {
    await withMockSecureServer((_req, res) => sendJson(res, 401, { error: "Invalid refresh token" }), async () => {
        await assert.rejects(() => refreshAccessTokenViaServer("bad_token"), /Invalid refresh token/);
    });
});
