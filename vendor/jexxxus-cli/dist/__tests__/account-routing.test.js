...
import { extractContactDeleteFromText, formatAccountRoutingHint, isContactDeletePrompt, isVaultPrimaryPrompt, isVaultReadOnlyPrompt, isVaultWritePrompt, planAccountTools, } from "../lib/blxckchat/account-routing.js";
...
test("create contact named Anna Test and assign phone captures contactName", () => {
    const prompt = "Create a contact named Anna Test and assign a random phone number + email.";
    const plan = planAccountTools(prompt);
    assert.equal(plan.contactName, "Anna Test");
    assert.equal(isVaultWritePrompt(prompt), true);
});
