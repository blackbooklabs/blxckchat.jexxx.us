...
export function getBibleBooks(section) {
    ...
    return entries
        .filter((e) => fs.statSync(path.join(sectionPath, e)).isDirectory() &&
        /^\d{2}-/.test(e))
        .sort();
}
...
function parseVerseFrontmatter(content) {
...
export function getChapter(section, book, chapter) {
...
export function normalizeBookLookupKey(bookName) {
    return bookName
        .toLowerCase()
        .replace(/^\d{2}-/, "")
        .replace(/['.]/g, "")
...
export function looksLikeVerseReference(query) {
...
export function parseVerseReference(query) {
    ...
    const match = trimmed.match(/^((?:\d+\s+)?[A-Za-z][A-Za-z0-9\s.'-]*?)\s+(\d+)\s*[: ]\s*(\d+)\s*$/);
...
export function findVerse(query) {
    ...
    try {
        const verses = getBibleVerses(bookInfo.section, bookInfo.book, chapter);
        const verseFile = verses.find((v) => v.startsWith(`${chapterNum}-${verseNum}`));
        if (!verseFile)
            return null;
