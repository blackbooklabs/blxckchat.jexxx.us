 ...
 */
export function ensureCredsDir() {
...
export function loadCredentials(options = {}) {
    try {
        ...
        if (mode !== 0o600) {
            if (!options.quiet) {
                console.warn(chalk.yellow(`⚠️  Warning: ~/.jexxxus/credentials.json has mode ${mode.toString(8)}, expected 0600. Fixing...`));
            }
        ...
        const creds = data.accessToken ? data : data.credentials;
        if (!creds) {
            return null;
        }
        return creds;
 ...
 */
export function shouldRefreshToken(creds) {
    const expiresAt = new Date(creds.expiresAt);
    const now = new Date();
 ...
 */
export function getTokenExpiryMinutes(creds) {
    const expiresAt = new Date(creds.expiresAt);
    const now = new Date();
 ...
 */
export async function ensureValidToken(refreshFn, options = {}) {
    const creds = loadCredentials(options);
    if (!creds) {
        throw new Error("Not authenticated. Run: " + chalk.cyan("jexxxus auth login"));
    }
    ...
    if ((expired || expiringSoon) && refreshFn) {
        try {
            ...
            if (expired) {
                throw new Error(`Token expired and refresh failed: ${detail}. Run /auth login or ` +
                    chalk.cyan("jexxxus auth login") +
                    " to re-authenticate.");
            }
            if (!options.quiet) {
                console.warn(chalk.yellow("⚠️  Failed to auto-refresh token:"), detail);
            }
            return creds;
        }
    }
    if (expired) {
        throw new Error("Token expired. Run /auth refresh, /auth login, or " +
            chalk.cyan("jexxxus auth login") +
            " to re-authenticate.");
    }
    return creds;
}
function mergeAuthProfile(existing, profile) {
    const merged = {};
...
function credentialsFromAuthResponse(data, existing = {}) {
...
export function formatAuthStatusLines(creds) {
    if (!creds) {
        return [
    ...
    const expiryLabel = expiryMinutes < 0
        ? "EXPIRED"
        : expiryMinutes < 5
...
export function openDeviceAuthBrowser(verificationUrl) {
    try {
 ...
 */
export async function pollDeviceAuth(userCode, codeVerifier, timeoutSeconds, pollIntervalMs = 2000) {
 ...
 */
export async function refreshAccessTokenViaServer(refreshToken) {
    ...
    if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        throw new Error(body?.error ?? `Token refresh failed (${response.status}). Run 'jexxxus auth login' again.`);
    }
    const data = (await response.json());
