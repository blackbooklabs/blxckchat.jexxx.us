...
export function fuzzyMatchContact(rows, contactName) {
    const needle = normalizeName(contactName);
    return (rows.find((r) => normalizeName(r.name) === needle) ??
        rows.find((r) => normalizeName(r.name).includes(needle)));
}
function truncateNotes(notes, max = 240) {
...
export async function fetchAccountSummary(session, asUserId) {
    ...
    const [bb, nxt, tv] = await Promise.all([
        fetchBlxckbookExport(bbVault.client, bbVault.effectiveUserId, session.creds.email),
...
async function fetchBlxckbookContacts(client, userId, opts) {
    let query = client
        .from("contacts")
        .select("*")
        .eq("user_id", userId)
...
async function fetchBlxckbookTimeline(client, userId, contactName, limit) {
    const exportData = await fetchBlxckbookExport(client, userId, "");
...
export async function executeAccountQuery(session, args) {
    ...
    switch (action) {
        ...
        case "export_preview": {
            ...
            if (includeBlxckbook) {
                payload.blxckbook = await fetchBlxckbookExport(bbVault.client, userId, session.creds.email);
            }
            if (includeNxt) {
        ...
        case "contacts": {
            const contactOpts = { limit };
            if (args.relationshipStatus) {
                contactOpts.relationshipStatus = args.relationshipStatus;
            }
            const contacts = await fetchBlxckbookContacts(bbVault.client, userId, contactOpts);
            const lines = contacts.map((c) => `• ${c.name}${c.relationshipStatus ? ` (${c.relationshipStatus})` : ""}` +
        ...
        case "contact": {
            ...
            if (target === "nxt") {
                ...
                const hit = fuzzyMatchContact(nxt.contacts.map((row) => ({ name: vesselName(row) })), contactName);
                const row = hit
        ...
        case "journal": {
            const entries = await fetchBlxckbookJournal(bbVault.client, userId, contactName, limit);
            if (entries.length === 0) {
                return contactName
                    ? `No journal entries linked to "${contactName}".`
                    : "No journal entries in your vault.";
            }
            const lines = entries.map((j) => `• ${j.date} — ${j.title}` +
                `${j.tags.length ? ` [${j.tags.join(", ")}]` : ""}` +
                `\n  ${truncateNotes(j.content, 160)}`);
            return `Journal entries (${entries.length}):\n${lines.join("\n\n")}`;
        }
        case "timeline": {
            const events = await fetchBlxckbookTimeline(bbVault.client, userId, contactName, limit);
            if (events.length === 0) {
                return contactName
        ...
        case "events": {
            ...
            if (contactName) {
                const profile = fuzzyMatchContact(nxt.contacts.map((row) => ({ name: vesselName(row) })), contactName);
                const vesselId = profile
                    ? nxt.contacts.find((r) => vesselName(r) === profile.name)?.id
                    : undefined;
                if (!vesselId) {
        ...
        case "playlists": {
            ...
            const lines = playlists.map((p) => `• ${p.name} — ${p.videoCount} video${p.videoCount === 1 ? "" : "s"}` +
        ...
        case "playlist": {
            ...
            const detail = await fetchPlaylistDetail(tvVault.client, userId, playlistName, limit);
            if (!detail) {
                return `No TV playlist matching "${playlistName}"${scopeLabel}.`;
            }
            const { playlist, videos } = detail;
            const lines = videos.map((v) => `• ${v.order}. ${v.title} (${v.videoId})`);
            return [
