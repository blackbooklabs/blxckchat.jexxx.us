...
export const bibleTool = {
    name: "bible_query",
    description: "Query the JEXXXUS Bible vault for SCRIPTURE ONLY. action='query' requires a verse " +
    ...
    async execute(args) {
        ...
        const wantsBooks = rawAction === "books" || (rawAction.includes("book") && !bookArg && !rawAction.includes("chapter"));
        ...
        if (wantsChapters) {
            ...
            return (`Chapter list for ${bookArg} (navigation only — not verse text):\n` +
                `${chapters.join(", ")}\n\n` +
        ...
        if (wantsVerse) {
            ...
            if (!looksLikeVerseReference(query)) {
                return (`This does not look like a scripture reference (expected Book Chapter:Verse, e.g. "1 John 1:9"). ` +
        ...
        return (`Error: could not determine what to look up from action="${rawAction}". ` +
