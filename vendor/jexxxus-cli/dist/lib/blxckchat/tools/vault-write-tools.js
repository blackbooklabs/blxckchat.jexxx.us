...
export const addContactTool = {
    ...
    async execute(args) {
        ...
        if (typeof args.relationshipStatus === "string")
...
export const updateContactTool = {
    name: "update_contact",
    description: "Update fields on one BLXCKBOOK contact or NXT vessel (fuzzy-matched by name). This writes " +
...
export const addJournalEntryTool = {
    name: "add_journal_entry",
    description: "Create a new BLXCKBOOK journal entry for the signed-in user, optionally linked to existing " +
    ...
    async execute(args) {
        ...
        if (Array.isArray(args.linkedContactNames)) {
            options.linkedContactNames = args.linkedContactNames;
        }
...
export const updateJournalEntryTool = {
    ...
    description: "Update an existing BLXCKBOOK journal entry's title, content, or tags — matched by id (if " +
    ...
    async execute(args) {
        ...
        if (!entryIdOrTitle)
            return "Error: entryIdOrTitle is required.";
        const resolved = await resolveAuthenticatedAccountSession();
        if (!resolved.ok)
            return `Error: ${resolved.message}`;
        const updates = {};
...
export const deleteJournalEntryTool = {
    ...
    description: "Permanently delete a BLXCKBOOK journal entry (and its contact links) — matched by id or " +
...
export const manageContactEventTool = {
    name: "manage_contact_event",
    description: "Create, update, or delete an NXT logged date/event (contact_events), linked to a vessel by " +
    ...
    async execute(args) {
        ...
        if (action === "create") {
            ...
            if (!contactName)
                return "Error: contactName is required for action=create.";
            if (!title || !eventDate)
                return "Error: title and eventDate are required for action=create.";
            const fields = { title, eventDate };
            if (typeof args.eventType === "string")
                fields.eventType = args.eventType;
        ...
        if (action === "update") {
            ...
            if (!eventId)
                return "Error: eventId is required for action=update.";
            const updates = {};
            if (typeof args.title === "string")
                updates.title = args.title;
            if (typeof args.eventDate === "string")
        ...
        const eventId = String(args.eventId ?? "").trim();
        if (!eventId)
            return "Error: eventId is required for action=delete.";
...
export const managePlaylistTool = {
    ...
    description: "Create, rename, delete a JEXXXUS | TV custom playlist, or add/remove a video from one. " +
    ...
    async execute(args) {
        ...
        if (!playlistName)
            return "Error: playlistName is required.";
        const resolved = await resolveAuthenticatedAccountSession();
        if (!resolved.ok)
            return `Error: ${resolved.message}`;
        const playlistOptions = {};
        if (typeof args.newName === "string")
            playlistOptions.newName = args.newName;
        if (typeof args.videoId === "string")
            playlistOptions.videoId = args.videoId;
        if (typeof args.isPrivate === "boolean")
            playlistOptions.isPrivate = args.isPrivate;
        const result = await managePlaylist(resolved.session, action, playlistName, playlistOptions);
        return result.message;
    },
};
export const exportVaultTool = {
    name: "export_vault",
    description: "Export the signed-in user's BLXCKBOOK and/or NXT vault data to a local JSON file. Defaults " +
...
export const syncExportFileTool = {
    name: "sync_export_file",
    description: "Read a local vault export JSON file (from export_vault, possibly hand-edited via " +
    ...
    async execute(args) {
        ...
        if (!resolved.ok)
            return `Error: ${resolved.message}`;
        const blxckbook = payload
            .blxckbook;
        if (!blxckbook) {
            return 'Error: expected a { "blxckbook": { "contacts": [...], "journal_entries": [...] } } shape.';
        }
        const summary = await syncBlxckbookExport(resolved.session, blxckbook);
        return summary;
    },
};
//# sourceMappingURL=vault-write-tools.js.map