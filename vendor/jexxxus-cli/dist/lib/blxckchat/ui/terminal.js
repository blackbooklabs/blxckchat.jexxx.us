...
import { formatHeroHint, formatHeroSubtitle, renderJexxxusHeroPlain, } from "./components/jexxxus-hero.js";
import { THEME } from "./theme.js";
import { createSession, addUserMessage, addAssistantMessage, addToolResult, updateToolResult, } from "./session/session-store.js";
...
import { StreamThinkingParser, formatLiveStreamDisplay, formatThinkingWaitState, } from "./renderer/stream-thinking.js";
import { extractThinkingBlocks } from "./components/thinking-block.js";
import { buildChromeDigestPlain, buildTuISnapshotWithChrome, } from "./renderer/plain-text.js";
import { copyToClipboard, getChromeDigestPath, getSnapshotPath, writeChromeDigest, writeSnapshot, } from "./session/tui-snapshot.js";
...
export async function startTerminalChat(provider, tools, options) {
    ...
    const session = options.resume
        ...
        : createSession();
    let activeConfig = { ...options.storedConfig };
    ...
    const showIdleHero = () => {
        ...
        tui.messageBox.showHero(renderJexxxusHeroPlain(Math.max(40, width - 4), heroMeta), heroMeta);
    };
    const refreshAuthChrome = () => {
    ...
    const copyLastReply = async () => {
        ...
        tui.statusBar.setMessage(copied ? "Last reply copied" : "Copy failed — see TUI snapshot");
    };
    const collectChromeDigestInput = () => {
        const liveCreds = loadCredentials({ quiet: true });
        const liveAuth = liveCreds
    ...
    const setActiveConfig = (config, nextProvider) => {
        activeConfig = config;
    ...
    const openModelPicker = async () => {
        ...
        const items = options.map((opt) => {
            const activeMarker = opt.id === activeConfig.model && opt.provider === activeConfig.provider
                ? "▸ "
                : "";
            return {
        ...
        const activeIdx = options.findIndex((o) => o.id === activeConfig.model && o.provider === activeConfig.provider);
        if (!tui.overlay)
            return;
    ...
    const openProviderPicker = async (catalogId) => {
        const { listProvidersRedacted, getProviderByName } = await import("../config.js");
        const { getCatalogEntry, listCatalogEntries } = await import("../providers/catalog.js");
        if (catalogId) {
        ...
        if (picked.id.startsWith("saved:")) {
            ...
            tui.messageBox.appendSystem(`Switched to "${resolved.name}" (${resolved.provider}/${resolved.model})`);
        }
    };
    const runProviderSetupFlow = async (entry) => {
        if (!tui.overlay)
            return;
        const { defaultModelFor, resolveBaseUrl, resolveEnvApiKey } = await import("../providers/catalog.js");
        const { listProvidersRedacted, getProviderByName, upsertProvider: upsertP, buildProviderConfig, } = await import("../config.js");
        const { listModelsForProvider } = await import("../providers/models.js");
        const { resolveProvider: resolveP } = await import("../providers/registry.js");
        let apiKey;
        let baseUrl;
        try {
            ...
            tui.messageBox.appendSystem(`Provider ready: "${built.name}" (${built.provider}/${built.model})`);
        }
        catch (err) {
            tui.messageBox.appendError(err instanceof Error ? err.message : "Provider setup failed");
        }
    };
    const openDivinityPicker = async () => {
        const { listDivinityPersonas, getDivinitiesSearchPaths } = await import("../divinities/source.js");
        const { activateDivinityPersona, formatDivinityActivationMessage } = await import("../divinities/session.js");
        const personas = listDivinityPersonas();
        if (personas.length === 0) {
            const checked = getDivinitiesSearchPaths()
                .map((base) => base + "/Personas")
                .join("\n  ");
        ...
        const activeIdx = items.findIndex((i) => i.id === (activeId ?? "__clear__"));
        const picked = await tui.overlay.showPicker(items, {
            title: "░ divinities ░",
            selectedIndex: activeIdx >= 0 ? activeIdx : 0,
        });
        ...
        if (picked.id === "__clear__") {
            ...
            tui.messageBox.appendSystem("Divinity cleared — BLXCKCHAT default agent restored.");
            return;
        }
        const persona = personas.find((p) => p.id === picked.id);
        if (!persona)
            return;
    ...
    const openAuthPicker = async () => {
        const { loadCredentials: loadCreds, getTokenExpiryMinutes, deleteCredentials, } = await import("../../auth.js");
        ...
        if (!creds) {
            ...
            const expiryStr = expiry < 0 ? "EXPIRED" : `${Math.floor(expiry)}m remaining`;
        ...
        if (picked.id === "login") {
            try {
                ...
                if (err instanceof Error && err.message === "cancelled") {
                    ...
                    tui.messageBox.appendError(`Login failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
            return;
        }
        if (picked.id === "logout") {
            deleteCredentials();
            refreshAuthChrome();
            tui.messageBox.appendSystem("Signed out. Run /auth login to connect secure.jexxx.us again.");
        }
    };
    const onDivinityChatCleared = () => {
        tui.messageBox.clearChat();
        showIdleHero();
    };
    ...
    const updateScrollStatus = () => {
        ...
        if (pinnedToBottom) {
            tui.statusBar.setMessage("Shift+↑↓ scroll · Ctrl+B history · / commands · Ctrl+Shift+Y chrome · ? hotkeys");
        }
        else {
            tui.statusBar.setMessage(`History ${percent}% · End jump latest · Ctrl+B scroll · ? hotkeys`);
        }
    };
    const runTurn = async (trimmed) => {
        isProcessing = true;
        ...
        tui.messageBox.updateAssistantStream(assistantBlockIndex, formatThinkingWaitState(), "", "");
        const streamBuffer = new StreamBuffer();
        const thinkingParser = new StreamThinkingParser();
        const pushStreamToUi = () => {
            const state = thinkingParser.getState();
            tui.messageBox.updateAssistantStream(assistantBlockIndex, formatLiveStreamDisplay(state), state.visible, state.thinking);
        };
        const showToolPending = (toolName) => {
            addToolResult(session, toolName, "Running...", "pending");
            tui.statusBar.setMessage(`⏳ Running tool: ${toolName}...`);
        };
        const showToolComplete = (toolName, result, status) => {
        ...
        const personaRecord = activeDivinity
            ...
            : null;
        const persona = personaRecord && activeDivinity
            ? {
        ...
        try {
            const { response, history } = await runAgent(activeProvider, tools, trimmed, session.conversationHistory, {
                ...
                confirmToolCall: (toolName, args) => tui.callbacks.onConfirmTool(toolName, args),
            });
            session.conversationHistory = history;
            thinkingParser.flush();
            const parsed = extractThinkingBlocks(response || streamBuffer.getContent());
            ...
            const visible = parsed.visibleContent || response || streamBuffer.getContent();
            addAssistantMessage(session, visible);
            tui.messageBox.finalizeAssistant(assistantBlockIndex, visible, mergedBlocks);
            maybeAutosave();
        }
        catch (err) {
            if (err instanceof AgentAbortedError) {
                ...
                tui.messageBox.cancelInFlightAssistant();
                tui.messageBox.appendError(`[ERROR] ${msg} — full trace: ~/.jexxxus/crash.log`);
            ...
            if (queued) {
                tui.statusBar.setMessage(messageQueue.length > 0
    ...
    const callbacks = {
        ...
        onCopySnapshot: () => void copySnapshot().then(({ path, copied }) => {
            tui.statusBar.setMessage(copied ? "TUI copied" : `Snapshot: ${path}`);
        }),
        onCopyChrome: () => void copyChromeDigest().then(({ path, copied }) => {
            tui.statusBar.setMessage(copied ? "Chrome copied" : `Chrome digest: ${path}`);
        }),
    ...
    const tui = createDyeTui({
        callbacks,
        subtitle: options.providerLabel ?? `${activeConfig.provider}/${activeConfig.model}`,
    });
    await tui.ready();
    tui.topBar.setSubtitle(options.providerLabel ?? `${activeConfig.provider}/${activeConfig.model}`);
    if (options.resume && session.messages.length > 0) {
        tui.messageBox.replaySession(session);
        tui.messageBox.appendSystem(`Resumed autosave (${session.messages.length} messages, ${session.conversationHistory.length} history turns).`);
    }
    else {
        showIdleHero();
    }
...
function isFreeTierZenModel(catalogId, modelId) {
...
function createAuthTuiActionsNoScreen(options) {
    return {
        async status() {
            const { formatAuthStatusLines, loadCredentials: lc } = await import("../../auth.js");
        ...
        async logout() {
            const { loadCredentials: lc, deleteCredentials: dc } = await import("../../auth.js");
        ...
        async refresh() {
            const { refreshAccessTokenViaServer, loadCredentials: lc, saveCredentials: sc, } = await import("../../auth.js");
            const creds = lc({ quiet: true });
            if (!creds)
