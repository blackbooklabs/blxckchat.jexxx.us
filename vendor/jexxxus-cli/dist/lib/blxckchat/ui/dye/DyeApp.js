...
import { AlternateScreen, Box, useInput, useApp, useWindowSize, useSelection, usePaste, measureElement, } from "@sauerapple/dye";
...
export const DyeApp = ({ store, callbacks, initialInputValue = "", overlayRef, }) => {
    ...
    const [filterSelectionStart, setFilterSelectionStart] = React.useState(null);
    // Search overlay cursor/selection state
    const [searchCursorPos, setSearchCursorPos] = React.useState(0);
    const [searchSelectionStart, setSearchSelectionStart] = React.useState(null);
    const pickerResolveRef = React.useRef(null);
    ...
    const [promptSelectionStart, setPromptSelectionStart] = React.useState(null);
    const [deviceLoginText, setDeviceLoginText] = React.useState(null);
    const deviceLoginResolveRef = React.useRef(null);
    React.useSyncExternalStore((cb) => store.subscribe(cb), () => store.getSnapshot());
    React.useEffect(() => {
        if (overlayRef) {
            overlayRef.current = {
                showPicker: (items, options) => new Promise((resolve) => {
                ...
                startDeviceLogin: async () => {
                    const { startDeviceAuth, pollDeviceAuth, openDeviceAuthBrowser } = await import("../../../auth.js");
                    const { userCode, codeVerifier, expiresIn, verificationUrl } = await startDeviceAuth();
                    openDeviceAuthBrowser(verificationUrl);
                    setDeviceLoginText(`Visit: ${verificationUrl}\nCode: ${userCode}`);
                    return new Promise((resolve, reject) => {
                        deviceLoginResolveRef.current = (val) => {
    ...
    const overlayActive = Boolean(store.confirmDialog || pickerState || promptState || deviceLoginText);
    ...
    const [messageAreaHeight, setMessageAreaHeight] = React.useState(undefined);
    React.useLayoutEffect(() => {
        if (!messageAreaRef.current)
            return;
        const { height } = measureElement(messageAreaRef.current);
        if (height > 0)
            setMessageAreaHeight(height);
    }, [termWidth, termHeight]);
    useMouseScroll({
        onScrollUp: callbacks.onScrollUp,
        onScrollDown: callbacks.onScrollDown,
    }, !overlayActive);
    const maxScrollLines = React.useMemo(() => store.blocks.flatMap((b) => (b.content || "").split("\n")).length, [store.blocks]);
    ...
    const wordBoundaryLeft = (text, pos) => {
        let p = pos;
        while (p > 0 && text[p - 1] === " ")
            p--;
        while (p > 0 && text[p - 1] !== " ")
    ...
    const wordBoundaryRight = (text, pos) => {
        let p = pos;
    ...
    useInput((input, key) => {
        ...
        if (slashSuggestions.length > 0) {
            if (key.upArrow) {
                setSlashSelectedIndex((i) => i <= 0 ? slashSuggestions.length - 1 : i - 1);
                return;
            }
            if (key.downArrow) {
                setSlashSelectedIndex((i) => i >= slashSuggestions.length - 1 ? 0 : i + 1);
                return;
            }
        ...
        if (key.ctrl && input === "s") {
            callbacks.onExportSession().then((path) => {
                if (path)
                    store.showToast(`Session exported`);
            });
            return;
        }
        if (key.ctrl && input === "n") {
        ...
        if (key.ctrl && input === "y") {
            if (key.shift)
                callbacks.onCopyChrome();
            else
                callbacks.onCopySnapshot();
            return;
        }
        if (key.meta && input === "z") {
    ...
    const handleSubmit = React.useCallback((line) => {
        if (!line.trim() || overlayActive)
            return;
        setInputValue("");
        store.inputValue = "";
