...
import { canRunBlessedTui, prepareStdinForTui, restoreTerminalForReadline, } from "./ui/tty.js";
...
function isSpuriousMouseInput(line) {
    if (line.length < 6)
        return false;
...
async function startReadlineFallback(provider, tools, providerLabel, storedConfig, resume) {
    ...
    if (resume && session.conversationHistory.length > 0) {
        console.log(chalk.dim(`[BLXCKCHAT] Resumed autosave (${session.conversationHistory.length} history messages).\n`));
    }
 ...
 */
export async function startInteractiveChat(provider, tools, options) {
    ...
    try {
        ...
        console.error(chalk.red(`[BLXCKCHAT] TUI failed to start: ${err instanceof Error ? err.message : "Unknown error"}`));
