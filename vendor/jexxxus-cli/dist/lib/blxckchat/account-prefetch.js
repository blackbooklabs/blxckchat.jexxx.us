 ...
 */
export async function prefetchAccountContext(userPrompt, session) {
    ...
    if (readOnly && plan.action) {
        try {
            ...
            if (plan.contactName)
                args.contactName = plan.contactName;
            if (plan.relationshipStatus)
                args.relationshipStatus = plan.relationshipStatus;
            if (plan.playlistName)
                args.playlistName = plan.playlistName;
            ...
            console.warn("[account-prefetch] live account_query failed:", err instanceof Error ? err.message : err);
        }
    }
    try {
...
//# sourceMappingURL=account-prefetch.js.map