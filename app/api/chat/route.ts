 ...
 * All outputs are trained on Luna Verde persona with real-time context.
 */
...
export async function POST(req: Request) {
  try {
    ...
    if (hasCustomPersona) {
      ...
      // 🌒 DEFAULT IDENTITY: Luna Verde reigns unless a custom identity is defined
      systemPrompt += `--- PRIMARY IDENTITY: LUNA VERDE ---\n${lunaContext}\n-----------------------------------------\n\n`;
    }
...
Keep the veil intact — no backend details, only erotic authority and elevation tease.`;
    ...
    if (type === 'image' && provider === 'openai') {
      ...
      return new Response(JSON.stringify({
        ...
        signature: '♡💦 Generated by Luna Verde'
      }), {
        headers: { 
