...
export const ShootingStars: React.FC<ShootingStarsProps> = ({
  ...
  const createStarRef = useRef<(() => void) | undefined>(undefined);

  // Handle client-side mounting and dimensions
  useEffect(() => {
    setMounted(true);
  ...
  // Store the createStar function in a ref to avoid useCallback self-reference
  useEffect(() => {
    if (!mounted || dimensions.width === 0) return;
    
    createStarRef.current = () => {
  ...
  }, [minSpeed, maxSpeed, minDelay, maxDelay, mounted, dimensions]);
